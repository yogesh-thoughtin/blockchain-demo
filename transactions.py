...
# Signatures immports
from signatures import generate_keys, verify, sign_message
...
class Transaction:
    ...
    def add_required(self, address,  private_key):
        '''
        A method to add additonal user's verification to the ledger.(Escrow transactions)
        address: Public address of escrow user.
        private: Private key of escrow user.
        returns: None
        Note: The use of private key is only for demo verification.\
            In real scenario private key is secret and only stored to escrow user.
        '''
...
if __name__ == "__main__":
    ...
    transaction.add_output(pu2,1)
    transaction.sign(pr1)
    transaction.add_required(pu4,pr4)
    ...
    transaction.sign(pr3)
    transaction.add_required(pu4,pr5)
